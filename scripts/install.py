...
REQUIRED_FILES = {
    ...
    "server.py": "backend",
    "agent.py": "backend",
    "uninstall.py": "scripts",
    "manifest.xml": ".",
    "cert.pem":"backend/certs",
    "key.pem":"backend/certs",
    "history.py":"backend",
    "generar_certificados.py": "backend/certs",
    "sidebar.html": "frontend/static",
    "client.js": "frontend/static"
}
...
def find_source_file(filename):
    ...
    if (cwd / filename).exists():
        return cwd / filename
    for child in cwd.rglob(filename):
        if child.is_file():
            return child
    parent = cwd.parent
    for child in parent.rglob(filename):
        if child.is_file():
            return child
        ...
        if src:
            ...
            if filename == "__init__.py":
                with open(target_dir / "__init__.py", "w") as f:
                    pass
            elif filename in ["cert.pem", "key.pem"]:
                pass 
            else:
                missing.append(filename)
...
def setup_python_env(dest_dir: Path):
    ...
    cert_path = dest_dir / "backend" / "certs" / "cert.pem"
    if not cert_path.exists():
        log_step("Generando Certificados SSL Locales...")
        gen_script = dest_dir / "backend" / "certs" / "generar_certificados.py"
        if gen_script.exists():
            subprocess.run([str(python_exe), str(gen_script)], cwd=str(gen_script.parent))
...
def create_shortcut_powershell(dest_dir: Path, desktop_dir: Path):
    ...
    launcher_script = dest_dir / "scripts" / "launcher.py"
